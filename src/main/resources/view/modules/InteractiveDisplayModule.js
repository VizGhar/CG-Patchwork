 ...
 */
function setScale(entity, scaleX, scaleY) {
    entity.container.scale.x = scaleX
    entity.container.scale.y = scaleY
}

/**
 * Set z index to entity
 * @param entity the entity to change zIndex to
 * @param zIndex to set to entity
 */
function setZindex(entity, zIndex) {
...
/**
 * Reset z index of entity
 * @param entity the entity to reset
 */
function resetZIndex(entity) {
    entity.container.zIndex = entity.currentState.zIndex
}
 ...
 * @param fraction of animation on interval (0, 1)
 */
function performTransformation(entity, parameters, fraction) {
    ...
    switch (interaction) {
        case DISPLAY:
            ...
            return
        case RESIZE:
            setScale(entity, actualScaleX, actualScaleY)
            if (zIndex !== -1) {
...
export class InteractiveDisplayModule {
    constructor(_assets) {
        ...
        this.lastProgress = 1
        this.lastFrame = 0
        this.running_animations = new Map()
    }
     ...
     */
    animateScene (delta) {
        // recompute animation percentage
        this.hovered_entities.forEach(entityId => {
            if (!this.running_animations.has(entityId)) {
                this.running_animations.set(entityId, 0.0)
            } else if (this.running_animations.get(entityId) < 1.0) {
                this.running_animations.set(entityId, this.running_animations.get(entityId) + delta / 200)
            }
        })

        this.running_animations = new Map(
            [...this.running_animations]
                .filter(([k, v]) => this.hovered_entities.has(k))
        );

        transformAllAssociated(this.hovered_entities, this, CLICK_ONLY, delta)
        transformAllAssociated(this.clicked_entities, this, HOVER_ONLY, delta)
    }
}
